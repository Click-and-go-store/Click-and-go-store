...
import { Heart, ShoppingCart, X, Plus, Minus, Search, Trash2, CreditCard, CheckCircle2, Truck, ShieldCheck, BadgePercent, Star, ArrowRight, RefreshCw, AlertTriangle } from "lucide-react";
...
const CREAM = "#F4F0E7";
const PAPER = "#FFFDF9";
const WHATSAPP_NUMBER = "573000000000"; // reemplaza por el número real de la tienda
      ...
      {panel && (
        <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", justifyContent: "flex-end" }}>
          ...
          <div style={{
              ...
              {panel === "cart" && (
                cart.length === 0 ? (
                  ...
                  <div style={{ textAlign: "center", padding: "60px 10px" }}>
                    <CheckCircle2 size={54} color={ORANGE} />
                    <div style={{ fontWeight: 800, marginTop: 14 }}>¡Pago realizado con éxito!</div>
                    <div style={{ fontSize: 13, color: "#8a8478", marginTop: 6 }}>Gracias por comprar en Click &amp; Go Store.</div>
                  </div>
                ) : (
                  cart.map(item => (
            ...
            {panel === "cart" && cart.length > 0 && !checkedOut && (
              <div style={{ padding: "18px 22px", borderTop: "1px solid #e6e0d3", background: PAPER }}>
                ...
                <button
                  onClick={handlePay}
                  style={{
                    width: "100%", background: ORANGE, color: "#fff", border: "none", borderRadius: 999,
                    padding: "14px 0", fontWeight: 800, fontSize: 14, cursor: "pointer",
                ...
                <a
                  href={whatsappHref} target="_blank" rel="noopener noreferrer"
                  style={{
                    marginTop: 10, width: "100%", background: "#25D366", color: "#fff", border: "none", borderRadius: 999,
                    padding: "12px 0", fontWeight: 700, fontSize: 13, cursor: "pointer", textDecoration: "none",
                    display: "flex", alignItems: "center", justifyContent: "center", gap: 8
                  }}
                >
                  <WhatsAppIcon size={16} /> Confirmar pedido por WhatsApp
                </a>
              </div>
            )}
          </div>
